...
- `risorse.py` — icona, loghi, dialogo "Informazioni" e splash d'avvio
  ...
  `_finestra_splash` costruisce (senza mostrarlo) il `QDialog` senza
  cornice con il pulsante "Chiudi" — niente chiusura a tempo, resta
  finché l'utente non lo chiude lui; `mostra_splash` lo apre (solo
  editor.py per ora).
