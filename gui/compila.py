...
a = Analysis(
    ...
    binaries=[],
    datas=[({str(avv)!r}, '.'),
           ({str(assets)!r}, os.path.join('gui', 'assets'))],
...
def prepara(mondo, build_dir: str, radice: Path = RADICE) -> tuple[str, str]:
    """Scrive ``avventura.json`` e il ``.spec`` in *build_dir*.
...
def compila(mondo, cartella_output: str, radice: Path = RADICE,
    ...
    work = build_dir / "work"
    try:
        spec, nome = prepara(mondo, str(build_dir), radice)
